...
router.get('/bd-watchers', (req: any, res: any) => {
  try {
    ...
    for (const ent of entries) {
      ...
      try {
        const files = getUserBdsFiles(rawUserId);
        const bds: UserBd[] = files.flatMap((f: string) =>
          parseBdsFromFile(fs.readFileSync(f, 'utf8'))
        );
        const match = bds.find(
          (b) =>
            b.title === targetTitle &&
            b.writer === targetWriter &&
            (b.readTimes ?? 0) >= 1
        );
        if (match) {
          results.push({
