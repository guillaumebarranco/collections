...
export class AddManwhaComponent {
  ...
  updateEntityField<K extends keyof AddManwhaEntityForm>(
    ...
    if (field === 'nbTomes') {
      ...
      nextValue = (Number.isNaN(asNumber) ? 0 : asNumber) as AddManwhaEntityForm[K];
    }
    this.entityForm.set({
      ...current,
      [field]: nextValue,
  ...
  updateUserField<K extends keyof AddManwhaUserForm>(
    ...
    if (field === 'rating' || field === 'readTimes') {
      const asNumber = Number(value);
      nextValue = (Number.isNaN(asNumber) ? 0 : asNumber) as AddManwhaUserForm[K];
    }
    this.userForm.set({
      ...current,
      [field]: nextValue,
  ...
  async onSubmit() {
    ...
    try {
      const response = await fetch(`${this.apiUrl}/manwhas/add`, {
        method: 'POST',
        headers: {
