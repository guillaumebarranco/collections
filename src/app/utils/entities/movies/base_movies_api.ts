import { BaseMovie } from '../../../models/movie-model';

export const baseMoviesApi: BaseMovie[] = [
  {
    title: "A Ghost Story",
    director: " David Lowery",
    actors: [
      {
        name: "Rooney Mara",
      },
      {
        name: "Casey Affleck",
      },
      {
        name: "Rob Morgan",
      },
      {
        name: "Will Oldham",
      },
      {
        name: "Liz Staub",
      },
      {
        name: "Kenneesaw",
      },
    ],
    coverUrl: '/movies_pictures/72f40728a446.jpg',
    releaseDate: "2017-07-07",
    length: 92,
    genre: ['Fantastique'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "The Punisher: One Last Kill",
    director: " Reinaldo Marcus Green",
    actors: [
      {
        name: "Jon Bernthal",
      },
      {
        name: "Jason Clarke",
      },
      {
        name: "Ebon Moss-Bachrach",
      },
      {
        name: "Ben Barnes",
      },
      {
        name: "Giorgia Whigham",
      },
      {
        name: "Josh Stewart",
      },
    ],
    coverUrl: '/movies_pictures/the_punisher_one_last_kill_1.webp',
    releaseDate: "2026-05-12",
    length: 51,
    genre: ['Action'],
    saga: "Marvel Cinematic Universe",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "Michael",
    director: "Antoine Fuqua",
    actors: [
      {
        name: "Jaafar Jackson",
      },
      {
        name: "Miles Teller",
      },
      {
        name: "Colman Domingo",
      },
      {
        name: "Laura Harrier",
      },
      {
        name: "Kellan Lutz",
      },
      {
        name: "Nafessa Williams",
      },
      {
        name: "Tamala Jones",
      },
    ],
    coverUrl: '/movies_pictures/70879e704be5.jpg',
    releaseDate: "2026-04-22",
    length: 127,
    genre: ['Biographie'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "Jim Queen",
    director: "Marco Nguyen, Nicolas Athané",
    actors: [
      {
        name: "Alex Ramirez",
      },
      {
        name: "Jérémy Gillet",
      },
      {
        name: "François Sagat",
      },
      {
        name: "Marion Cotillard",
      },
      {
        name: "Vincent Lacoste",
      },
      {
        name: "Adèle Haenel",
      },
    ],
    coverUrl: '/movies_pictures/0f7896f12a65.jpg',
    releaseDate: "2026-06-17",
    length: 80,
    genre: ['Comédie', 'Animation'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['France'],
    selectDisplayOrder: 0,
  },

  {
    title: "Fjord",
    director: "Cristian Mungiu",
    actors: [
      {
        name: "Sebastian Stan",
      },
      {
        name: "Renate Reinsve",
      },
      {
        name: "Nikolaj Lie Kaas",
      },
      {
        name: "Elliott Crosset Hove",
      },
      {
        name: "Sofie Gråbøl",
      },
      {
        name: "Lene Maria Christensen",
      },
    ],
    coverUrl: '/movies_pictures/784531c5c7d4.jpg',
    releaseDate: "2026-08-19",
    length: 146,
    genre: ['Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['Norvège', 'Roumanie'],
    selectDisplayOrder: 0,
  },

  {
    title: "Her Private Hell",
    director: "Nicolas Winding Refn",
    actors: [
      {
        name: "Sophie Thatcher",
      },
      {
        name: "Charles Melton",
      },
      {
        name: "Diego Calva",
      },
      {
        name: "Havana Rose Liu",
      },
      {
        name: "Jack Kilmer",
      },
      {
        name: "Tom Rhys Harries",
      },
    ],
    coverUrl: '/movies_pictures/bebaade5bcc5.jpg',
    releaseDate: "",
    length: 109,
    genre: ['Science Fiction', 'Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis', 'Danemark'],
    selectDisplayOrder: 0,
  },

  {
    title: "Hope",
    director: "Na Hong-jin",
    actors: [
      {
        name: "Jung-Min Hwang",
      },
      {
        name: "In-sung Zo",
      },
      {
        name: "Jung Ho-Yeon",
      },
      {
        name: "Michael Fassbender",
      },
      {
        name: "Alicia Vikander",
      },
      {
        name: "Park Hae-joon",
      },
      {
        name: "Yoon Je-moon",
      },
    ],
    coverUrl: '/movies_pictures/316a7f4961c3.jpg',
    releaseDate: "",
    length: 140,
    genre: ['Science Fiction', 'Fantastique', 'Horreur'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['Corée du Sud'],
    selectDisplayOrder: 0,
  },

  {
    title: "Paper Tiger",
    director: "James Gray",
    actors: [
      {
        name: "Miles Teller",
      },
      {
        name: "Adam Driver",
      },
      {
        name: "Scarlett Johansson",
      },
      {
        name: "Michael Gandolfini",
      },
      {
        name: "Liev Schreiber",
      },
      {
        name: "Teyonah Parris",
      },
    ],
    coverUrl: '/movies_pictures/7c9da7dd2b00.jpg',
    releaseDate: "",
    length: 115,
    genre: ['Thriller', 'Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "Le Château d'Arioka",
    director: "Kiyoshi Kurosawa",
    actors: [
      {
        name: "Ryōta Miyadate",
      },
      {
        name: "Masahiro Motoki",
      },
      {
        name: "Masaki Suda",
      },
      {
        name: "Munetaka Aoki",
      },
      {
        name: "Yuriko Yoshitaka",
      },
      {
        name: "Joe Odagiri",
      },
      {
        name: "Tasuku Emoto",
      },
    ],
    coverUrl: '/movies_pictures/f8084b2e106c.jpg',
    releaseDate: "",
    length: 147,
    genre: ['Historique'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['Japon'],
    selectDisplayOrder: 0,
  },

  {
    title: "Diamond",
    director: "Andy Garcia",
    actors: [
      {
        name: "Andy Garcia",
      },
      {
        name: "Danny Huston",
      },
      {
        name: "Brendan Fraser",
      },
      {
        name: "Dustin Hoffman",
      },
      {
        name: "Bill Muray",
      },
      {
        name: "Rosemarie Dewitt",
      },
      {
        name: "Bill Murray",
      },
    ],
    coverUrl: '/movies_pictures/da906b8bfce7.jpg',
    releaseDate: "",
    length: 116,
    genre: ['Policier'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "La Bataille de Gaulle : L’âge de fer",
    director: "Antonin Baudry",
    actors: [
      {
        name: "Simon Abkarian",
      },
      {
        name: "Florian Lesieur",
      },
      {
        name: "Benoît Magimel",
      },
      {
        name: "Mathieu Kassovitz",
      },
      {
        name: "Niels Schneider",
      },
      {
        name: "Anamaria Vartolomei",
      },
    ],
    coverUrl: '/movies_pictures/2855890bb215.jpg',
    releaseDate: "2026-06-03",
    length: 160,
    genre: ['Historique', 'Guerre', 'Biographie'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['France'],
    selectDisplayOrder: 0,
  },

  {
    title: "Roma Elastica",
    director: "Bertrand Mandico",
    actors: [
      {
        name: "Marion Cotillard",
      },
      {
        name: "Noémie Merlant",
      },
      {
        name: "Vincent Macaigne",
      },
      {
        name: "Elina Löwensöhn",
      },
      {
        name: "Pauline Étienne",
      },
      {
        name: "Jean-Baptiste Morillon",
      },
    ],
    coverUrl: '/movies_pictures/265723a40f7a.webp',
    releaseDate: "2026-12-23",
    length: 107,
    genre: ['Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['France', 'Italie'],
    selectDisplayOrder: 0,
  },

  {
    title: "L'Inconnue",
    director: "Arthur Harari",
    actors: [
      {
        name: "Léa Seydoux",
      },
      {
        name: "Niels Schneider",
      },
      {
        name: "Vincent Macaigne",
      },
      {
        name: "Adèle Exarchopoulos",
      },
      {
        name: "Jean-Pierre Darroussin",
      },
      {
        name: "Sara Giraudeau",
      },
    ],
    coverUrl: '/movies_pictures/8bd6673bf2da.webp',
    releaseDate: "2026-08-26",
    length: 160,
    genre: ['Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['France'],
    selectDisplayOrder: 0,
  },

  {
    title: "Le Vertige",
    director: "Quentin Dupieux",
    actors: [
      {
        name: "Alain Chabat",
      },
      {
        name: "Jonathan Cohen",
      },
      {
        name: "Anaïs Demoustier",
      },
      {
        name: "Vincent Macaigne",
      },
      {
        name: "Benoît Poelvoorde",
      },
      {
        name: "Gringe",
      },
    ],
    coverUrl: '/movies_pictures/bc88653611e1.jpg',
    releaseDate: "2026-06-10",
    length: 66,
    genre: ['Comédie', 'Science Fiction'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['France'],
    selectDisplayOrder: 0,
  },

  {
    title: "La Bola Negra",
    director: "Javier Calvo, Javier Ambrossi",
    actors: [
      {
        name: "Guitarricadelafuente",
      },
      {
        name: "Carlos González",
      },
      {
        name: "Miguel Bernardeau",
      },
      {
        name: "Penélope Cruz",
      },
      {
        name: "Milo Quifes",
      },
      {
        name: "Glenn Close",
      },
    ],
    coverUrl: '/movies_pictures/cddcc2444f92.jpg',
    releaseDate: "",
    length: 154,
    genre: ['Drame', 'Historique'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['Espagne'],
    selectDisplayOrder: 0,
  },

  {
    title: "The End of It",
    director: "Maria Martínez Bayona",
    actors: [
      {
        name: "Rebecca Hall",
      },
      {
        name: "Noomie Rapace",
      },
      {
        name: "Gael Garcia Bernal",
      },
      {
        name: "Gael García Bernal",
      },
      {
        name: "Stellan Skarsgård",
      },
      {
        name: "Nikolaj Lie Kaas",
      },
      {
        name: "Clara Rugaard",
      },
    ],
    coverUrl: '/movies_pictures/7af12c4520c2.jpg',
    releaseDate: "",
    length: 102,
    genre: ['Science Fiction', 'Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['Royaume-Uni'],
    selectDisplayOrder: 0,
  },

  {
    title: "Histoires de la Nuit",
    director: "Léa Mysius",
    actors: [
      {
        name: "Bastien Bouillon",
      },
      {
        name: "Benoît Magimel",
      },
      {
        name: "Hafsia Herzi",
      },
      {
        name: "Monica Bellucci",
      },
      {
        name: "Vincent Lacoste",
      },
      {
        name: "Adèle Exarchopoulos",
      },
    ],
    coverUrl: '/movies_pictures/f185e5587a69.jpg',
    releaseDate: "2026-09-16",
    length: 114,
    genre: ['Thriller'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['France'],
    selectDisplayOrder: 0,
  },

  {
    title: "Victorian Psycho",
    director: "Zachary Wigon",
    actors: [
      {
        name: "Maika Monroe",
      },
      {
        name: "Ruth Wilson",
      },
      {
        name: "Thomasin McKenzie",
      },
      {
        name: "Jason Isaacs",
      },
      {
        name: "Amy De Bhrún",
      },
      {
        name: "Jacobi Jupe",
      },
      {
        name: "Katherine Moran",
      },
    ],
    coverUrl: '/movies_pictures/9974bbdb125e.jpg',
    releaseDate: "",
    length: 90,
    genre: ['Horreur'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis', 'Royaume-Uni'],
    selectDisplayOrder: 0,
  },

  {
    title: "L'Objet du Délit",
    director: "Agnès Jaoui",
    actors: [
      {
        name: "Agnès Jaoui",
      },
      {
        name: "Daniel Auteuil",
      },
      {
        name: "Eye Haidara",
      },
      {
        name: "Claire Chust",
      },
      {
        name: "Oussama Kheddam",
      },
      {
        name: "Lucie Gallo",
      },
    ],
    coverUrl: '/movies_pictures/3fe30017be8f.jpg',
    releaseDate: "2026-05-27",
    length: 133,
    genre: ['Comédie'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['France'],
    selectDisplayOrder: 0,
  },

  {
    title: "Mandalorian & Grogu",
    director: "Jon Favreau",
    actors: [
      {
        name: "Pedro Pascal",
      },
      {
        name: "Segourney Weaver",
      },
      {
        name: "Jeremy Allen White",
      },
      {
        name: "Sigourney Weaver",
      },
      {
        name: "Bill Burr",
      },
      {
        name: "Giancarlo Esposito",
      },
      {
        name: "Emily Swallow",
      },
    ],
    coverUrl: '/movies_pictures/2d9327df78e4.jpg',
    releaseDate: "2026-05-20",
    length: 132,
    genre: ['Action', 'Science Fiction', 'Aventure'],
    saga: "Star Wars",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "Ladies First",
    director: "Thea Sharrock",
    actors: [
      {
        name: "Sacha Baron Cohen",
      },
      {
        name: "Rosamund Pike",
      },
      {
        name: "Richard E. Grant",
      },
      {
        name: "Emily Mortimer",
      },
      {
        name: "Charles Dance",
      },
      {
        name: "Fiona Shaw",
      },
      {
        name: "Tom Davis",
      },
    ],
    coverUrl: '/movies_pictures/acd0f921b4e3.jpg',
    releaseDate: "2026-05-22",
    length: 93,
    genre: ['Comédie'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "Scary Movie (6)",
    director: "Michael Tiddes",
    actors: [
      {
        name: "Marlon Wayans",
      },
      {
        name: "Shawn Wayans",
      },
      {
        name: "Anna Faris",
      },
      {
        name: "Regina Hall",
      },
      {
        name: "Simon Rex",
      },
      {
        name: "Craig Wayans",
      },
      {
        name: "Affion Crockett",
      },
    ],
    coverUrl: '/movies_pictures/f71af794912a.jpg',
    releaseDate: "2026-06-03",
    length: 96,
    genre: ['Comédie'],
    saga: "Scary Movie",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "Who",
    director: "Wil Aime",
    actors: [
      {
        name: "Wil Aime",
      },
      {
        name: "Pierre Niney",
      },
      {
        name: "Adèle Exarchopoulos",
      },
      {
        name: "Vincent Lacoste",
      },
      {
        name: "Gilles Lellouche",
      },
      {
        name: "François Civil",
      },
    ],
    coverUrl: '/movies_pictures/ed73601b85b9.jpg',
    releaseDate: "2026-05-28",
    length: 130,
    genre: ['Thriller'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['France'],
    selectDisplayOrder: 0,
  },

  {
    title: "L'Abandon",
    director: "Vincent Garenq",
    actors: [
      {
        name: "Antoine Reinartz",
      },
      {
        name: "Emmanuelle Bercot",
      },
      {
        name: "Vincent Lacoste",
      },
      {
        name: "India Hair",
      },
      {
        name: "Jean-Pierre Darroussin",
      },
      {
        name: "Adèle Haenel",
      },
    ],
    coverUrl: '/movies_pictures/1b1ec9782ecd.jpg',
    releaseDate: "2026-05-13",
    length: 100,
    genre: ['Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['France'],
    selectDisplayOrder: 0,
  },

  {
    title: "Ceux qui comptent",
    director: "Jean-Baptiste Leonetti",
    actors: [
      {
        name: "Sandrine Kiberlain",
      },
      {
        name: "Pierre Lottin",
      },
      {
        name: "Vincent Lacoste",
      },
      {
        name: "Jean-Pierre Darroussin",
      },
      {
        name: "Denis Podalydès",
      },
      {
        name: "Zabou Breitman",
      },
    ],
    coverUrl: '/movies_pictures/b973d5ee21da.jpg',
    releaseDate: "2026-03-25",
    length: 98,
    genre: ['Comédie', 'Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['France'],
    selectDisplayOrder: 0,
  },

  {
    title: "The Bride!",
    director: "Maggie Gyllenhaal",
    actors: [
      {
        name: "Jessie Buckley",
      },
      {
        name: "Christian Bale",
      },
      {
        name: "Peter Sarsgaard",
      },
      {
        name: "Annette Bening",
      },
      {
        name: "Julianne Moore",
      },
      {
        name: "Josh Hartnett",
      },
    ],
    coverUrl: '/movies_pictures/c59cf1742a94.jpg',
    releaseDate: "2026-03-06",
    length: 126,
    genre: ['Horreur', 'Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "The World of Love",
    director: "Yoon Ga-eun",
    actors: [
      {
        name: "Seo Su-bin",
      },
      {
        name: "Jang Hye-jin",
      },
      {
        name: "Kim Go-eun",
      },
      {
        name: "Park Hae-joon",
      },
      {
        name: "Lee Jung-eun",
      },
      {
        name: "Kim Mi-kyung",
      },
    ],
    coverUrl: '/movies_pictures/4b800d685331.jpg',
    releaseDate: "2025-10-22",
    length: 119,
    genre: ['Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['Corée du Sud'],
    selectDisplayOrder: 0,
  },

  {
    title: "Plus fort que moi",
    director: "Kirk Jones",
    actors: [
      {
        name: "Robert Aramayo",
      },
      {
        name: "Shirley Henderson",
      },
      {
        name: "James McArdle",
      },
      {
        name: "Thomasin McKenzie",
      },
      {
        name: "Adrian Lester",
      },
      {
        name: "Amrita Acharia",
      },
    ],
    coverUrl: '/movies_pictures/90d2c928bfb0.jpg',
    releaseDate: "2026-04-01",
    length: 121,
    genre: ['Biographie', 'Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['Royaume-Uni'],
    selectDisplayOrder: 0,
  },

  {
    title: "Rue Málaga",
    director: "Maryam Touzani",
    actors: [
      {
        name: "Carmen Maura",
      },
      {
        name: "Marta Etura",
      },
      {
        name: "Nadia Kounda",
      },
      {
        name: "Hafsia Herzi",
      },
      {
        name: "Vincent Lacoste",
      },
      {
        name: "Niels Schneider",
      },
    ],
    coverUrl: '/movies_pictures/012aa766ab5a.jpg',
    releaseDate: "2025-01-01",
    length: 110,
    genre: ['Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['Maroc', 'France'],
    selectDisplayOrder: 0,
  },

  {
    title: "La fille du train",
    director: "Tate Taylor",
    actors: [
      {
        name: "Emily Blunt",
      },
      {
        name: "Rebecca Ferguson",
      },
      {
        name: "Haley Bennett",
      },
      {
        name: "Justin Theroux",
      },
      {
        name: "Luke Evans",
      },
      {
        name: "Allison Janney",
      },
      {
        name: "Lisa Kudrow",
      },
    ],
    coverUrl: '/movies_pictures/6ceb7ad2fcdf.jpg',
    releaseDate: "2016-10-26",
    length: 112,
    genre: ['Thriller'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "Fabricant de larmes",
    director: "Alessandro Genovesi",
    actors: [
      {
        name: "Caterina Ferioli",
      },
      {
        name: "Simone Baldasseroni",
      },
      {
        name: "Chiara Primaria",
      },
      {
        name: "Antonio Griffo",
      },
      {
        name: "Eugenia Turilli",
      },
      {
        name: "Giuseppe Maggio",
      },
    ],
    coverUrl: '/movies_pictures/e8e216b1735e.jpeg',
    releaseDate: "2024-04-04",
    length: 103,
    genre: ['Romance', 'Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['Italie'],
    selectDisplayOrder: 0,
  },

  {
    title: "Aucun autre choix",
    director: "Park Chan-wook",
    actors: [
      {
        name: "Lee Byung-hun",
      },
      {
        name: "Son Ye-jin",
      },
      {
        name: "Park Hee-soon",
      },
      {
        name: "Lee Sung-min",
      },
      {
        name: "Kim Min-jae",
      },
      {
        name: "Jo Woo-jin",
      },
    ],
    coverUrl: '/movies_pictures/d63213f345d6.jpg',
    releaseDate: "2026-02-11",
    length: 139,
    genre: ['Thriller', 'Comédie'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['Corée du Sud'],
    selectDisplayOrder: 0,
  },

  {
    title: "Un jour avec mon père",
    director: "Akinola Davies Jr.",
    actors: [
      {
        name: "Ṣọpẹ́ Dìrísù",
      },
      {
        name: "Godwin Chiemerie Egbo",
      },
      {
        name: "Gbemisola Ikumelo",
      },
      {
        name: "Wunmi Mosaku",
      },
      {
        name: "Kobna Holdbrook-Smith",
      },
      {
        name: "Franc Ashman",
      },
    ],
    coverUrl: '/movies_pictures/71e7926a151d.jpg',
    releaseDate: "2026-03-26",
    length: 94,
    genre: ['Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['Royaume-Uni', 'Nigeria'],
    selectDisplayOrder: 0,
  },

  {
    title: "Die, My Love",
    director: "Lynne Ramsay",
    actors: [
      {
        name: "Jennifer Lawrence",
      },
      {
        name: "Robert Pattinson",
      },
      {
        name: "LaKeith Stanfield",
      },
      {
        name: "Amy Adams",
      },
      {
        name: "Chloë Sevigny",
      },
      {
        name: "Barry Keoghan",
      },
    ],
    coverUrl: '/movies_pictures/373f654a29dd.jpg',
    releaseDate: "2026-04-29",
    length: 119,
    genre: ['Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "Ce qu'il reste de nous",
    director: "Cherien Dabis",
    actors: [
      {
        name: "Saleh Bakri",
      },
      {
        name: "Cherien Dabis",
      },
      {
        name: "Elia Suleiman",
      },
      {
        name: "Hiam Abbass",
      },
      {
        name: "Ali Suliman",
      },
      {
        name: "Kamel El Basha",
      },
    ],
    coverUrl: '/movies_pictures/1f28413d0981.webp',
    releaseDate: "2026-03-11",
    length: 145,
    genre: ['Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['Palestine', 'États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "À contre-sens",
    director: "Domingo González",
    actors: [
      {
        name: "Nicole Wallace",
      },
      {
        name: "Gabriel Guevara",
      },
      {
        name: "Martina Carrió",
      },
      {
        name: "Iván Sánchez",
      },
      {
        name: "Goya Toledo",
      },
      {
        name: "Victor Varona",
      },
      {
        name: "Martina D'Antiochia",
      },
    ],
    coverUrl: '/movies_pictures/310306cec8f6.jpg',
    releaseDate: "2023-06-08",
    length: 118,
    genre: ['Drame', 'Romance'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['Espagne'],
    selectDisplayOrder: 0,
  },

  {
    title: "À contre-sens : Londres",
    director: "Charlotte Fassler, Dani Girdwood",
    actors: [
      {
        name: "Asha Banks",
      },
      {
        name: "Matthew Broome",
      },
      {
        name: "Freddy Carter",
      },
      {
        name: "Madeline Cumber",
      },
      {
        name: "Jayda Eyles",
      },
      {
        name: "Lara Rossi",
      },
    ],
    coverUrl: '/movies_pictures/e5bf731054f2.jpg',
    releaseDate: "2025-02-13",
    length: 119,
    genre: ['Drame', 'Romance'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['Royaume-Uni'],
    selectDisplayOrder: 0,
  },

  {
    title: "Saltburn",
    director: "Emerald Fennell",
    actors: [
      {
        name: "Barry Keoghan",
      },
      {
        name: "Jacob Elordi",
      },
      {
        name: "Rosamund Pike",
      },
      {
        name: "Richard E. Grant",
      },
      {
        name: "Archie Madekwe",
      },
      {
        name: "Carey Mulligan",
      },
      {
        name: "Paul Rhys",
      },
    ],
    coverUrl: '/movies_pictures/51abd0166810.jpg',
    releaseDate: "2023-11-17",
    length: 131,
    genre: ['Thriller', 'Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['Royaume-Uni'],
    selectDisplayOrder: 0,
  },

  {
    title: "Chien de la casse",
    director: "Jean-Baptiste Durand",
    actors: [
      {
        name: "Raphaël Quenard",
      },
      {
        name: "Jean-Baptiste Durand",
      },
      {
        name: "Adèle Exarchopoulos",
      },
      {
        name: "Francis Renaud",
      },
      {
        name: "Tom Leeb",
      },
      {
        name: "India Hair",
      },
      {
        name: "Clémence Sabbagh",
      },
    ],
    coverUrl: '/movies_pictures/0a3168c616a1.jpg',
    releaseDate: "2023-04-19",
    length: 93,
    genre: ['Comédie', 'Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['France'],
    selectDisplayOrder: 0,
  },

  {
    title: "Oiseaux de paradis",
    director: "Sarah Adina Smith",
    actors: [
      {
        name: "Kristine Frøseth",
      },
      {
        name: "Diana Silvers",
      },
      {
        name: "Carla Langley",
      },
      {
        name: "Paul Wesley",
      },
      {
        name: "Ebony Obsidian",
      },
      {
        name: "Gail Soltys",
      },
    ],
    coverUrl: '/movies_pictures/9791a5b241cc.jpg',
    releaseDate: "2021-09-24",
    length: 113,
    genre: ['Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "Kadaver",
    director: "Jarand Herdal",
    actors: [
      {
        name: "Gitte Witt",
      },
      {
        name: "Thomas Gullestad",
      },
      {
        name: "Thorbjørn Harr",
      },
      {
        name: "Maria Granlund",
      },
      {
        name: "Jonatan Rodriguez",
      },
      {
        name: "Tage Bautz-Forrest",
      },
    ],
    coverUrl: '/movies_pictures/a83793b129c5.jpg',
    releaseDate: "2020-10-22",
    length: 86,
    genre: ['Horreur'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['Norvège'],
    selectDisplayOrder: 0,
  },

  {
    title: "Je veux juste en finir",
    director: "Charlie Kaufman",
    actors: [
      {
        name: "Jessie Buckley",
      },
      {
        name: "Jesse Plemons",
      },
      {
        name: "Toni Collette",
      },
      {
        name: "David Thewlis",
      },
      {
        name: "Colby Minifie",
      },
      {
        name: "Abby McEnany",
      },
    ],
    coverUrl: '/movies_pictures/71f2a44e32c1.jpg',
    releaseDate: "2020-09-04",
    length: 134,
    genre: ['Drame', 'Thriller'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "Doctor Sleep",
    director: "Mike Flanagan",
    actors: [
      {
        name: "Ewan McGregor",
      },
      {
        name: "Rebecca Ferguson",
      },
      {
        name: "Kyliegh Curran",
      },
      {
        name: "Cliff Curtis",
      },
      {
        name: "Carl Lumbly",
      },
      {
        name: "Emily Alyn Lind",
      },
      {
        name: "Jocelin Donahue",
      },
    ],
    coverUrl: '/movies_pictures/ae5e879f088f.jpg',
    releaseDate: "2019-11-08",
    length: 152,
    genre: ['Horreur'],
    saga: "",
    description: "",
    fromEntity: { entityType: "book", title: "Doctor Sleep", secondEntityKey: "Stephen King" },
    countryOrigin: ['États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "Paradise Hills",
    director: "Alice Waddington",
    actors: [
      {
        name: "Emma Roberts",
      },
      {
        name: "Danielle Macdonald",
      },
      {
        name: "Awkwafina",
      },
      {
        name: "Milla Jovovich",
      },
      {
        name: "Jeremy Irvine",
      },
      {
        name: "Eiza González",
      },
    ],
    coverUrl: '/movies_pictures/e3ac1c612d61.jpg',
    releaseDate: "2019-08-30",
    length: 95,
    genre: ['Science Fiction'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['Espagne'],
    selectDisplayOrder: 0,
  },

  {
    title: "Première Année",
    director: "Thomas Lilti",
    actors: [
      {
        name: "Vincent Lacoste",
      },
      {
        name: "Antoine Reinartz",
      },
      {
        name: "Reda Kateb",
      },
      {
        name: "Paul Kircher",
      },
      {
        name: "Louise Bourgoin",
      },
      {
        name: "Philippine Velge",
      },
    ],
    coverUrl: '/movies_pictures/a1938a458a49.jpg',
    releaseDate: "2018-09-12",
    length: 105,
    genre: ['Comédie', 'Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['France'],
    selectDisplayOrder: 0,
  },

  {
    title: "Manchester by the Sea",
    director: "Kenneth Lonergan",
    actors: [
      {
        name: "Casey Affleck",
      },
      {
        name: "Michelle Williams",
      },
      {
        name: "Kyle Chandler",
      },
      {
        name: "Lucas Hedges",
      },
      {
        name: "Gretchen Mol",
      },
      {
        name: "C.J. Wilson",
      },
      {
        name: "Matthew Broderick",
      },
    ],
    coverUrl: '/movies_pictures/d6e244b6deaf.jpg',
    releaseDate: "2016-11-18",
    length: 137,
    genre: ['Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "Melancholia",
    director: "Lars von Trier",
    actors: [
      {
        name: "Kirsten Dunst",
      },
      {
        name: "Charlotte Gainsbourg",
      },
      {
        name: "Kiefer Sutherland",
      },
      {
        name: "Alexander Skarsgård",
      },
      {
        name: "Stellan Skarsgård",
      },
      {
        name: "Charlotte Rampling",
      },
      {
        name: "John Hurt",
      },
    ],
    coverUrl: 'Melancholia',
    releaseDate: "2011-05-26",
    length: 136,
    genre: ['Drame', 'Science Fiction'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['Danemark'],
    selectDisplayOrder: 0,
  },

  {
    title: "Une vie volée",
    director: "James Mangold",
    actors: [
      {
        name: "Winona Ryder",
      },
      {
        name: "Angelina Jolie",
      },
      {
        name: "Brittany Murphy",
      },
      {
        name: "Clea DuVall",
      },
      {
        name: "Elisabeth Moss",
      },
      {
        name: "Whoopi Goldberg",
      },
      {
        name: "Jared Leto",
      },
    ],
    coverUrl: '/movies_pictures/acd67cb359c8.jpg',
    releaseDate: "1999-12-21",
    length: 127,
    genre: ['Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "Quand Harry rencontre Sally",
    director: "Rob Reiner",
    actors: [
      {
        name: "Billy Crystal",
      },
      {
        name: "Meg Ryan",
      },
      {
        name: "Carrie Fisher",
      },
      {
        name: "Bruno Kirby",
      },
      {
        name: "Steven Ford",
      },
      {
        name: "Lisa Jane Persky",
      },
      {
        name: "Tracy Reiner",
      },
    ],
    coverUrl: '/movies_pictures/43e292fa7628.jpg',
    releaseDate: "1989-07-21",
    length: 96,
    genre: ['Comédie', 'Romance'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "Marcel le coquillage (avec ses chaussures)",
    director: "Dean Fleischer Camp",
    actors: [
      {
        name: "Jenny Slate",
      },
      {
        name: "Rosa Salazar",
      },
      {
        name: "Isabella Rossellini",
      },
      {
        name: "Thomas Middleditch",
      },
      {
        name: "Paul Sun-Hyung Lee",
      },
      {
        name: "Lesley Davison",
      },
    ],
    coverUrl: '/movies_pictures/b7a752f38069.jpg',
    releaseDate: "2021-09-03",
    length: 90,
    genre: ['Comédie', 'Animation'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "Mommy",
    director: "Xavier Dolan",
    actors: [
      {
        name: "Anne Dorval",
      },
      {
        name: "Antoine Olivier Pilon",
      },
      {
        name: "Suzanne Clément",
      },
      {
        name: "Patrick Huard",
      },
      {
        name: "Natalie Hamel",
      },
      {
        name: "Isabelle McAlpine",
      },
    ],
    coverUrl: '/movies_pictures/594426aed075.jpg',
    releaseDate: "2014-09-19",
    length: 138,
    genre: ['Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['Canada'],
    selectDisplayOrder: 0,
  },

  {
    title: "La Faille",
    director: "Gregory Hoblit",
    actors: [
      {
        name: "Anthony Hopkins",
      },
      {
        name: "Ryan Gosling",
      },
      {
        name: "Rosamund Pike",
      },
      {
        name: "David Strathairn",
      },
      {
        name: "Embeth Davidtz",
      },
      {
        name: "Billy Burke",
      },
      {
        name: "Bob Gunton",
      },
    ],
    coverUrl: '/movies_pictures/d89cd27049db.jpg',
    releaseDate: "2007-05-09",
    length: 112,
    genre: ['Thriller'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "Crocodile Dundee",
    director: "Peter Faiman",
    actors: [
      {
        name: "Paul Hogan",
      },
      {
        name: "Linda Kozlowski",
      },
      {
        name: "John Meillon",
      },
      {
        name: "David Gulpilil",
      },
      {
        name: "Steve Rackman",
      },
      {
        name: "Gerry Skilton",
      },
      {
        name: "Ritchie Singer",
      },
    ],
    coverUrl: '/movies_pictures/d33311228929.jpg',
    releaseDate: "1986-04-30",
    length: 97,
    genre: ['Comédie', 'Aventure'],
    saga: "Crocodile Dundee",
    description: "",
    fromEntity: null,
    countryOrigin: ['Australie', 'États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "Crocodile Dundee 2",
    director: "John Cornell",
    actors: [
      {
        name: "Paul Hogan",
      },
      {
        name: "Linda Kozlowski",
      },
      {
        name: "Ernie Dingo",
      },
      {
        name: "Hechter Ushkowitz",
      },
      {
        name: "Kenneth Mars",
      },
      {
        name: "Steve Rackman",
      },
      {
        name: "Gerry Skilton",
      },
    ],
    coverUrl: '/movies_pictures/c79ed332dda4.jpg',
    releaseDate: "1988-05-25",
    length: 111,
    genre: ['Comédie', 'Aventure'],
    saga: "Crocodile Dundee",
    description: "",
    fromEntity: null,
    countryOrigin: ['Australie', 'États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "Crocodile Dundee 3",
    director: "Simon Wincer",
    actors: [
      {
        name: "Paul Hogan",
      },
      {
        name: "Linda Kozlowski",
      },
      {
        name: "Serge Cockburn",
      },
      {
        name: "Alec Wilson",
      },
      {
        name: "Jonathan Bauman",
      },
      {
        name: "Steve Rackman",
      },
      {
        name: "Gerry Skilton",
      },
    ],
    coverUrl: '/movies_pictures/fe990d84d0a3.webp',
    releaseDate: "2001-04-20",
    length: 95,
    genre: ['Comédie', 'Aventure'],
    saga: "Crocodile Dundee",
    description: "",
    fromEntity: null,
    countryOrigin: ['Australie', 'États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "Disclosure Day",
    director: "Steven Spielberg",
    actors: [
      {
        name: "Josh O'Connor",
      },
      {
        name: "Emily Blunt",
      },
      {
        name: "Colin Firth",
      },
      {
        name: "Colman Domingo",
      },
      {
        name: "Eve Hewson",
      },
      {
        name: "Wyatt Russel",
      },
    ],
    coverUrl: "https://www.critikat.com/wp-content/uploads/2026/06/Disclosure-Day-affiche.jpg",
    releaseDate: "2026-06-10",
    length: 145,
    genre: ['Science Fiction'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis'],
    selectDisplayOrder: 0,
  },

  {
    title: "Regretting You",
    director: "Josh Boone",
    actors: [
      {
        name: "McKenna Grace",
      },
      {
        name: "Mason Thames",
      },
      {
        name: "Allison Williams",
      },
      {
        name: "Dave Franco",
      },
    ],
    coverUrl: "https://fr.web.img6.acsta.net/img/4c/92/4c92464185c7a72bc6260dd9ec1051e8.jpg",
    releaseDate: "2025-10-29",
    length: 117,
    genre: ['Comédie', 'Romance', 'Drame'],
    saga: "",
    description: "",
    fromEntity: null,
    countryOrigin: ['États-Unis'],
    selectDisplayOrder: 0,
  },
];
